...
(async()=>{
  ...
  context.fetch=async()=>({ok:true,status:200,json:async()=>({can_edit:true,compilation:{valid:false,blockers:[{id:1}]},rules:[{id:1,name:'rule',enabled:1,compilation:{included:true,compatible:false,issue:'<img src=x> unsupported'}}]})});
  await context.loadPolicies();
  assert.match(element('policy-notice').textContent,/1 rule\(s\) block compilation/);
  assert.match(element('policy-table-body').innerHTML,/Blocked:/);
  assert(!element('policy-table-body').innerHTML.includes('<img'));
  run("policyRows=[{id:1,name:'<img src=x>',src_iface:'ethernet1/1',vsys:3,enabled:1,proto:'tcp',action:'permit'}]");
  element('rule-proto').options=[{value:'tcp'}];element('rule-action').options=[{value:'permit'}];
  const rule=run('policyRows[0]');context.editRule(rule);
  assert.equal(element('rule-name').value,'<img src=x>');assert.equal(element('rule-src-iface').value,'ethernet1/1');
  assert.equal(element('rule-vsys').value,3);
  context.cloneRule(rule);assert.equal(element('rule-enabled').checked,false);assert.equal(element('rule-edit-id').value,'');
  let ruleBody;
  context.fetch=async(path,opts)=>{ruleBody=JSON.parse(opts.body);return {ok:false,status:422,json:async()=>({detail:'Invalid rule'})};};
  await context.saveRule();
  assert.equal(ruleBody.enabled,false);assert.equal(ruleBody.src_iface,'ethernet1/1');assert.equal(ruleBody.vsys,3);
  assert.match(element('rule-msg').textContent,/Invalid rule/);assert.equal(element('rule-save').disabled,false);
