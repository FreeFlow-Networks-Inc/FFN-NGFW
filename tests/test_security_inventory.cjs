...
(async()=>{
  ...
  try {
    ...
    assert.deepEqual(names,['management','shared-name','shared-name','intrazone-default','interzone-default']);
    assert(!(await page.locator('#pw-list').innerText()).includes('other-tenant'));
    await page.locator('#pw-search').fill('no matching rule');await count(3);
    ...
    await page.keyboard.press('Escape');await page.locator('#pw-search').fill('');await count(5);
    ...
    await page.locator('#modal-rule').waitFor({state:'hidden'});await count(5);
    assert.equal(writes.length,1);assert.equal(writes[0].p,'/api/policy/rules/2');assert(reads>=2);
    await page.locator('#pw-source').selectOption('running');await count(4);
    ...
    await page.locator('#pw-source').selectOption('candidate');await count(5);
    await page.locator('#pw-scope').selectOption('vsys2');await count(5);
    ...
    failure='xml';await page.locator('#pw-refresh').click();await count(4);
    ...
    failure='';rules[1].name='<img src=x onerror="window.injected=true">';await page.locator('#pw-refresh').click();await count(5);
