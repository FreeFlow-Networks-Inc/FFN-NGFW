...
class Barrier(unittest.TestCase):
    def test_actual_commit_handler_drains_before_writing_and_unlocks_on_failure(self):
        ...
        handler=next(n for n in ast.parse(source.read_text()).body
                     if isinstance(n,ast.AsyncFunctionDef) and n.name=='config_commit')
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            scope=dict(config_mgr=config,_sync_netresources_to_xml=sync,app=app,
            ...
            req=SimpleNamespace(partial_xpath=None,description='',commit_type='full')
            with self.assertRaises(HTTPException) as failure:
                asyncio.run(scope['config_commit'](req,{'username':'admin'}))
            self.assertEqual(failure.exception.status_code,409)
            self.assertEqual(calls,['drain','unlock'])
            calls.clear()
            app.state.platform_policy_guard=lambda data:calls.append('drain')
            with self.assertRaises(AtWrite):asyncio.run(scope['config_commit'](req,{'username':'admin'}))
